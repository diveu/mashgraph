...
//structure to use filters
template <typename ValueT>
struct Filter {
    Matrix<ValueT> kernel;
    unsigned int vert_radius, hor_radius;
	uint lbp, colors;
 
    Filter(Matrix<ValueT>& kern, unsigned int vertical, unsigned int horizontal,  unsigned int lb=0)
    {
        kernel = kern;
        vert_radius = vertical;
        hor_radius = horizontal;
		lbp = lb;
    }
 
    ValueT operator () (const Matrix<ValueT>& neighbourhood) const
    {
        ValueT res = static_cast<ValueT>(0);
 
        for (uint i = 0; i < neighbourhood.n_rows; i++) {
            for (uint j = 0; j < neighbourhood.n_cols; j++) {
				if (lbp == 0 && colors == 0) {
					res += kernel(i, j) * neighbourhood(i, j);
				}
				else {
					if (lbp){
						uint middle = neighbourhood.n_rows / 2;
						if (neighbourhood(i, j) < neighbourhood(middle, middle)){
							res += kernel(i, j);
						}
					}
				}
